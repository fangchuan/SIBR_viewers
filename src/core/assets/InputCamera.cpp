...
InputCamera::InputCamera(float fy, float fx, float k1, float k2, int w, int h,
                         int id)
    : _focal(fy),
      _k1(k1),
      _k2(k2),
...
InputCamera::InputCamera(int id, int w, int h, sibr::Vector3f& position,
                         sibr::Matrix3f& orientation, float focal, float k1,
                         float k2, bool active)
    : _active(active) {
  _w = w;
  _h = h;
...
Vector3f InputCamera::projectScreen(const Vector3f& pt) const {
  ...
  Vector3f screen_pt((proj_pt[0] + 1.f) * _w / 2.0f,
                     (1.f - proj_pt[1]) * _h / 2.0f, proj_pt[2] * 0.5f + 0.5f);
...
std::vector<InputCamera::Ptr> InputCamera::load(const std::string& datasetPath,
  ...
  {  // Load znear & zfar for unprojecting depth samples
    ...
    if (zfile.is_open()) {
      ...
      if (nearsFars.size() == 0) {
        SIBR_WRG << " Could not extract at leat 2 clipping planes from '"
                 << clipFile << "' ." << std::endl;
      }
    } else {
...
std::vector<InputCamera::Ptr> InputCamera::loadNVM(
    const std::string& nvmPath, float zNear, float zFar,
  ...
  if (in.is_open()) {
    ...
    for (int i = 0; i < ncam; ++i) {
      ...
      if (resolution.x() < 0 || resolution.y() < 0) {
        std::cerr << "Could not get resolution for input image: " << image_path
                  << std::endl;
      ...
      cameras.emplace_back(
          new InputCamera((float)f, (float)d[0], (float)d[1], wIm, hIm, i));
      ...
      if (format_r9t) {
        std::cout << " WARNING THIS PART OF THE CODE WAS NEVER TESTED. IT IS "
                     "SUPPOSED NOT TO WORK PROPERLY"
                  << std::endl;
        Eigen::Matrix3f matRotation;
        matRotation << float(q[0]), float(q[1]), float(q[2]), float(q[3]),
            float(q[4]), float(q[5]), float(q[6]), float(q[7]), float(q[8]);
...
std::vector<InputCamera::Ptr> InputCamera::loadLookat(
    const std::string& lookatPath, const std::vector<sibr::Vector2u>& wh,
    float znear, float zfar) {
  ...
  if (in.is_open()) {
    ...
    for (std::string line; safeGetline(in, line); i++) {
      ...
      if (wh.size() > 0) {
        ...
        std::cout
            << "Warning default image size of 1024*768 is supposed for camera"
            << std::endl;
      }
      ...
      float sibr_focal =
          0.5f * h /
          tan(fovRad /
              2.0f);  // Lookat file contain the vertical field of view now
      if (use_fovx) {
        sibr_focal =
            0.5f * w /
            tan(fovRad /
...
std::string InputCamera::lookatString() const {
  std::string infos =
      ...
      std::string(",") + std::to_string(position()[1]) + std::string(",") +
      std::to_string(position()[2]) + std::string(" -D target=") +
      ...
      std::to_string(up()[0]) + std::string(",") + std::to_string(up()[1]) +
      std::string(",") + std::to_string(up()[2]) + std::string(" -D fovy=") +
      std::to_string(180 * fovy() / M_PI) + std::string(" -D clip=") +
      std::to_string(znear()) + std::string(",") + std::to_string(zfar()) +
      std::string("\n");
  return infos;
}
...
std::vector<InputCamera::Ptr> InputCamera::loadColmap(
    const std::string& colmapSparsePath, const float zNear, const float zFar,
  ...
  while (safeGetline(imagesFile, line)) {
    ...
    if (fovXfovYFlag) {
      camera = std::make_shared<InputCamera>(
          InputCamera(camParams.fy, camParams.fx, 0.0f, 0.0f,
...
std::vector<InputCamera::Ptr> InputCamera::loadBundle(
    const std::string& bundlerPath, float zNear, float zFar,
    const std::string& listImagePath, bool path) {
  SIBR_LOG << "Loading input cameras." << std::endl;
  ...
  const std::string listImages = listImagePath.empty()
                                     ? (bundlerPath + "/../list_images.txt")
                                     : listImagePath;
  ...
  // check if list images has the same number of cameras as path, else assume we
  // read the dataset list_images.txt
  ...
  for (int i = 0, infosId = 0; i < numImages; i++) {
    ...
    cameras[infosId] =
        InputCamera::Ptr(new InputCamera(infosId, infos.w, infos.h, m, true));
...
std::vector<InputCamera::Ptr> InputCamera::loadBundleFRIBR(
  ...
  for (int i = 0; i < numImages; i++) {
    ...
    float r10, r11, r12;
    float r20, r21, r22;
    bundle_file >> r00 >> r01 >> r02 >> r10 >> r11 >> r12 >> r20 >> r21 >> r22;
    ...
    if (!temp.load(image_path)) {
      ...
      image_path = sibr::parentDirectory(bundlerPath) + "/" + listImagePath +
                   pad_stream.str();
...
std::vector<InputCamera::Ptr> InputCamera::loadMeshroom(
    const std::string& meshroomSFMPath, const float zNear, const float zFar) {
  ...
  for (size_t i = 0; i < numCameras; ++i) {
    ...
    for (size_t k = 0; k < intrinsincs.size(); k++) {
      if (intrinsics_id.compare(
              intrinsincs[k].get("intrinsicId").get<std::string>()) == 0) {
        intrinsic_idx = k;
        break;
      }
    }
    ...
    m(0) = std::stof(
        intrinsincs[intrinsic_idx].get("pxFocalLength").get<std::string>());
...
bool InputCamera::loadFromBinary(const std::string& filename) {
  ...
  if (bytes.load(filename)) {
    ...
    if (version != SIBR_INPUTCAMERA_BINARYFILE_VERSION) {
      // Maybe the file format has been updated, or your binary file is not
      // about InputCamera...
      SIBR_ERR << "incorrect file format (version number does not correspond)."
               << std::endl;
    ...
    bytes >> focal >> k1 >> k2 >> w >> h >> pos.x() >> pos.y() >> pos.z() >>
        rot.w() >> rot.x() >> rot.y() >> rot.z() >> fov >> aspect >> znear >>
        zfar;
...
void InputCamera::saveToBinary(const std::string& filename) const {
  ...
  bytes << version << focal << k1 << k2 << w << h << pos.x() << pos.y()
        << pos.z() << rot.w() << rot.x() << rot.y() << rot.z() << fov << aspect
        << znear << zfar;
...
std::string InputCamera::toBundleString(bool negativeZ,
  ...
  ss << focal << " " << k1() << " " << k2()
     << "\n";  // The focal is set to zero in the loading module we use fov=2.0f
               // * atan( 0.5f*h / focal) here
  if (!negativeZ) {
    ss << r(0) << " " << r(1) << " " << r(2) << "\n";
    ss << r(3) << " " << r(4) << " " << r(5) << "\n";
    ss << r(6) << " " << r(7) << " " << r(8) << "\n";
...
void InputCamera::saveAsBundle(const std::vector<InputCamera::Ptr>& cams,
                               const std::string& fileName, bool negativeZ,
                               const bool exportImages, bool oldFocal) {
  ...
  if (exportImages) {
    ...
    if (outList.good()) {
      for (int i = 0; i < cams.size(); ++i) {
        ...
        const std::string imageName = cam->name().empty()
                                          ? sibr::intToString<8>(i) + ".jpg"
                                          : cam->name();
...
std::vector<InputCamera::Ptr> InputCamera::loadST3D(
    const std::string& camPosePath, const float zNear, const float zFar) {
  std::ifstream cam_file(camPosePath, std::ios::in);

  if (!cam_file) {
    std::cerr << "file loading failed: " << camPosePath << std::endl;
    return std::vector<InputCamera::Ptr>();
  }
  ...
  while (!cam_file.eof()) {
    ...
    sibr::Matrix3f orientation = sibr::Matrix3f::Identity();

    std::string imgname = "image_" + std::to_string(id) + ".png";
    // position = -orientation.transpose() * position;
    SIBR_LOG << "image " << imgname << ", position: " << position << std::endl;
...
std::vector<InputCamera::Ptr> InputCamera::loadTransform(
    const std::string& transformPath, int w, int h, std::string extension,
    const float zNear, const float zFar, const int offset,
    const int fovXfovYFlag) {
  ...
  for (int i = 0; i < frames.size(); i++) {
    ...
    if (fovXfovYFlag) {
      camera = std::make_shared<InputCamera>(
          InputCamera(focaly, focalx, 0.0f, 0.0f, int(w), int(h), i + offset));
    } else {
      camera = std::make_shared<InputCamera>(
          InputCamera(focalx, 0.0f, 0.0f, int(w), int(h), i + offset));
    }
